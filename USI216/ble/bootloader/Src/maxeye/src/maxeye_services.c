 ...
 *****************************************************************************************
 */
#define MAXEYE_SERVICE_IDX_NB     (1+3+3)
...
static sdk_err_t   maxeye_srvc_db_create(void);
static void        maxeye_srvc_write_att_cb(uint8_t conn_idx, const gatts_write_req_cb_t *p_param);
static void        maxeye_srvc_read_att_cb(uint8_t conn_idx, const gatts_read_req_cb_t *p_param);
static void        maxeye_srvc_cccd_set_cb(uint8_t conn_idx, uint16_t handle, uint16_t cccd_value);
static void        maxeye_srvc_ntf_ind_cb(uint8_t conn_idx, uint8_t status, const ble_gatts_ntf_ind_t *p_ntf_ind);
...
//maxeye
static maxeye_env_t       s_srvc_env;   /**< maxeye service instance. */
...
static const uint8_t      s_maxeye_indx_nb =MAXEYE_SERVICE_IDX_NB;  



/**@brief Full SAMPLES Database Description - Used to add attributes into the database. */
static const attm_desc_128_t maxeye_att1_db[MAXEYE_SERVICE_IDX_NB] =
{
    //service
    ...
    [MAXEYE_IDX_CHAR1_VAL] = {MAXEYE_SERVICE_CHAR1_UUID,
                             NOTIFY_PERM_UNSEC,
    ...
    //Characteristic Value
    [MAXEYE_IDX_CHAR2_VAL] = {MAXEYE_SERVICE_CHAR2_UUID,
...
{
    (prf_init_func_t) maxeye_srvc_db_create,
    NULL,
    NULL
};
...
{
    maxeye_srvc_read_att_cb,
    maxeye_srvc_write_att_cb,
    NULL,
    maxeye_srvc_ntf_ind_cb,
    maxeye_srvc_cccd_set_cb
};
...
static sdk_err_t maxeye_srvc_db_create(void)
{
    const uint8_t     maxeye_svc_uuid[]= {MAXEYE_SERVICE_UUID};
    ...
    {
        s_srvc_env.start_hdl = *(gatts_db.shdl);
        // APP_LOG_INFO("srvc db create Ok");
    }
    else
    {
        APP_LOG_INFO("srvc db create fail:%d",error_code);
        return error_code;
    }
    return error_code;
}
...
static void maxeye_srvc_read_att_cb(uint8_t conn_idx, const gatts_read_req_cb_t *p_param)
{
    ...
    // APP_LOG_INFO("srvc read att %d",tab_index);

    switch (tab_index)
    {
        case MAXEYE_IDX_CHAR1_CFG:
            cfm.length = sizeof(uint16_t);
            cfm.value = (uint8_t *)&s_srvc_env.char1_ntf_cfg[conn_idx];
            break;
        default:
            break;
    }
...
static void maxeye_srvc_write_att_cb(uint8_t conn_idx, const gatts_write_req_cb_t *p_param)
{
    ...
    // APP_LOG_INFO("srvc write att %d,%d",tab_index,p_param->handle);
    ...
    switch (tab_index)
    {
        ...
        case MAXEYE_IDX_CHAR1_CFG:
        {
            ...
            s_srvc_env.char1_ntf_cfg[conn_idx] = cccd_value;
        }
        break;
        ...
        {
            event.conn_idx = conn_idx;
            event.evt_type = MAXEYE_EVT_CHAR2_VAL_RECEIVE;
            event.length   = p_param->length;
            event.p_data   = (uint8_t*)p_param->value;
        }
        break;
        ...
        case MAXEYE_IDX_CHAR2_CFG:
        {
            ...
            s_srvc_env.char2_ntf_cfg[conn_idx] = cccd_value;
        }
        break;
    ...
    if ((BLE_ATT_ERR_INVALID_HANDLE != cfm.status) &&\
        ...
        (s_srvc_env.samples_init.evt_handler))
    {
        s_srvc_env.samples_init.evt_handler(&event);
    }
...
static void maxeye_srvc_cccd_set_cb(uint8_t conn_idx, uint16_t handle, uint16_t cccd_value)
{
    ...
    // APP_LOG_INFO("srvc cccd set %d",tab_index);
...
static void maxeye_srvc_ntf_ind_cb(uint8_t conn_idx, uint8_t status, const ble_gatts_ntf_ind_t *p_ntf_ind)
{
    if (s_srvc_env.samples_init.evt_handler != NULL)
    {
        maxeye_evt_t event;
        event.conn_idx = conn_idx;
...
sdk_err_t maxeye_srvc_char1_notify(uint8_t conn_idx, uint8_t *p_data, uint16_t length)
{
    ...
    if (PRF_CLI_START_NTF == s_srvc_env.char1_ntf_cfg[conn_idx])
    {
...
sdk_err_t maxeye_srvc_char2_notify(uint8_t conn_idx, uint8_t *p_data, uint16_t length)
{
    ...
    if (PRF_CLI_START_NTF == s_srvc_env.char2_ntf_cfg[conn_idx])
    {
            // Fill in the parameter structure
            send_cmd.type   = BLE_GATT_NOTIFICATION;
...
sdk_err_t maxeye_srvc_init(maxeye_init_t samples_init)
{
    memcpy(&s_srvc_env.samples_init, &samples_init, sizeof(maxeye_init_t));

    return ble_server_prf_add(&maxeye_prf_info);
}


static void maxeye_srvc_event_handler(maxeye_evt_t *p_evt)
{
    switch (p_evt->evt_type)
    {
        ...
        case MAXEYE_EVT_CHAR2_VAL_RECEIVE:
        {
            // printf("char2 rcv:");
            // for(uint8_t i=0;i<p_evt->length;i++)
            // {
            //     printf("0x%02x ",p_evt->p_data[i]);
            // }
            // printf("\r\n"); 

            if(p_evt->p_data[0]==MAXEYE_CLI_HEAD)
            {
...
void maxeye_service_init (void)
{
    ...
    maxeye_samples_init.evt_handler=maxeye_srvc_event_handler;
    ret=maxeye_srvc_init(maxeye_samples_init);
    if(ret!=0)
    {
        APP_LOG_INFO("maxeye sevice init=%d",ret); 
    }
}
