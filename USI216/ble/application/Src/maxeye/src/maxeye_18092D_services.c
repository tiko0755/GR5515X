...
#define MAXEYE_SERVER1_IDX_NB     (3*3+1)
...
static sdk_err_t   maxeye_srvc1_db_create(void);
static void        maxeye_srvc1_write_att_cb(uint8_t conn_idx, const gatts_write_req_cb_t *p_param);
static void        maxeye_srvc1_read_att_cb(uint8_t conn_idx, const gatts_read_req_cb_t *p_param);
static void        maxeye_srvc1_cccd_set_cb(uint8_t conn_idx, uint16_t handle, uint16_t cccd_value);
static void        maxeye_srvc1_ntf_ind_cb(uint8_t conn_idx, uint8_t status, const ble_gatts_ntf_ind_t *p_ntf_ind);
...
//maxeye
static maxeye_env_t       s_srvc1_env;   /**< maxeye service instance. */
...
static const uint8_t      s_maxeye_indx_nb =MAXEYE_SERVER1_IDX_NB;  
// static uint8_t            maxeyeCharbuff[2] ={'5','!'} ;



/**@brief Full SAMPLES Database Description - Used to add attributes into the database. */
static const attm_desc_128_t maxeye_att1_db[MAXEYE_SERVER1_IDX_NB] =
{
    //service
    ...
    [MAXEYE_IDX_CHAR1_VAL] = {MAXEYE_SERVER1_CHAR1_UUID,
                             NOTIFY_PERM_UNSEC|WRITE_REQ_PERM_UNSEC|WRITE_CMD_PERM_UNSEC,
    ...
    // Characteristic Value
    [MAXEYE_IDX_CHAR2_VAL] = {MAXEYE_SERVER1_CHAR2_UUID,
                             READ_PERM_UNSEC,
                             (ATT_VAL_LOC_USER | ATT_UUID_TYPE_SET(UUID_TYPE_128)),
                             MAXEYE_MAX_DATA_LEN },
    // Characteristic - Client Characteristic Configuration Descriptor
    ...
    // Characteristic Value
    [MAXEYE_IDX_CHAR3_VAL] = {MAXEYE_SERVER1_CHAR3_UUID,
                             READ_PERM_UNSEC,
                             (ATT_VAL_LOC_USER | ATT_UUID_TYPE_SET(UUID_TYPE_128)),
                             MAXEYE_MAX_DATA_LEN },
    // Characteristic - Client Characteristic Configuration Descriptor
...
{
    (prf_init_func_t) maxeye_srvc1_db_create,
    NULL,
    NULL
};
...
{
    maxeye_srvc1_read_att_cb,
    maxeye_srvc1_write_att_cb,
    NULL,
    maxeye_srvc1_ntf_ind_cb,
    maxeye_srvc1_cccd_set_cb
};
...
static sdk_err_t maxeye_srvc1_db_create(void)
{
    const uint8_t     maxeye_svc_uuid[]= {MAXEYE_SERVICE1_UUID};
    ...
    {
        s_srvc1_env.start_hdl = *(gatts_db.shdl);
        // logX("srvc1 db create Ok");
    }
    else
    {
        logX("srvc1 db create fail:%d",error_code);
        return error_code;
    }
    return error_code;
}
...
static void maxeye_srvc1_read_att_cb(uint8_t conn_idx, const gatts_read_req_cb_t *p_param)
{
    ...
    uint8_t          tab_index = 0;
    const uint16_t   char_ntf_cfg = PRF_CLI_STOP_NTFIND;
    gatts_read_cfm_t cfm;
    

    tab_index = prf_find_idx_by_handle(handle,
                                       s_srvc1_env.start_hdl,
                                       s_maxeye_indx_nb, 
    ...
    // logX("srvc1 read att %d",tab_index);

    switch (tab_index)
    {
        case MAXEYE_IDX_CHAR1_CFG:
            cfm.length = sizeof(uint16_t);
            cfm.value = (uint8_t *)&s_srvc1_env.char1_ntf_cfg[conn_idx];
            break;
        case MAXEYE_IDX_CHAR2_CFG:
            cfm.length = sizeof(uint16_t);
            cfm.value = (uint8_t *)&char_ntf_cfg;
            break;    
        ...
        case MAXEYE_IDX_CHAR2_VAL:
            cfm.length = sizeof(uint8_t); 
            bValue=battLevel;
            cfm.value =(uint8_t *)&bValue;
            break;    

        case MAXEYE_IDX_CHAR3_VAL:
            cfm.length = sizeof(uint8_t);
            bValue=(battChargeStatus==BATT_CHARGEING_ENABLE)?EN_CHARGE_STATUS:DIS_CHARGE_STATUS;
            cfm.value = (uint8_t *)&bValue;
            break;
...
static void maxeye_srvc1_write_att_cb(uint8_t conn_idx, const gatts_write_req_cb_t *p_param)
{
    ...
    // logX("srvc1 write att %d",tab_index);
    ...
    switch (tab_index)
    {
        ...
        case MAXEYE_IDX_CHAR1_CFG:
            ...
            s_srvc1_env.char1_ntf_cfg[conn_idx] = cccd_value;
            break;
    ...
    if ((BLE_ATT_ERR_INVALID_HANDLE != cfm.status) &&\
        ...
        (s_srvc1_env.samples_init.evt_handler))
    {
        s_srvc1_env.samples_init.evt_handler(&event);
    }
...
static void maxeye_srvc1_cccd_set_cb(uint8_t conn_idx, uint16_t handle, uint16_t cccd_value)
{
    ...
    // logX("srvc1 cccd set %d",tab_index);
...
{
    logX("<%s >", __func__);
    if (s_srvc1_env.samples_init.evt_handler != NULL)
    {
        maxeye_evt_t event;
        event.conn_idx = conn_idx;
...
sdk_err_t maxeye_srvc1_notify(uint8_t conn_idx, uint8_t *p_data, uint16_t length)
{
    ...
    if (PRF_CLI_START_NTF == s_srvc1_env.char1_ntf_cfg[conn_idx])
    {
...
sdk_err_t maxeye_srvc1_init(maxeye_init_t samples_init)
{
    memcpy(&s_srvc1_env.samples_init, &samples_init, sizeof(maxeye_init_t));

    return ble_server_prf_add(&maxeye_prf_info);
}

static void maxeye_srvc1_event_handler(maxeye_evt_t *p_evt)
{
    logX("<%s evt_type:%d>", __func__, p_evt->evt_type);

    switch (p_evt->evt_type)
    {
        case MAXEYE_EVT_CHAR1_VAL_RECEIVE:
        {
            logX_raw("<%s p_data:0x", __func__);
            for(uint8_t i=0;i<p_evt->length;i++)
            {
                logX_raw("%02x ",p_evt->p_data[i]);
            }
            logX_raw(">\r\n");

            if(p_evt->p_data[0]==0x2C)
            {
                srvc1_write_attr_parse(&p_evt->p_data[0],p_evt->length);
            }
            else if(p_evt->p_data[0]==0x2F)//产测相关  
            {
                maxeye_product_test_handler(&p_evt->p_data[0],p_evt->length);
            }
            else if(p_evt->p_data[p_evt->length-2]=='\r' && p_evt->p_data[p_evt->length-1]=='\n'){
                const char* line = (const char*)p_evt->p_data;
                int32_t i;
                if(strncmp(line, "help", strlen("help"))==0){
                    logX("[Todo]should show help message here\r\n");
                    return;
                }
                else if(sscanf(line, "log_conf %d", &i)==1){
                    logConfig(i);
                    return;
                }
            }
        ...
        {
            logX("<%s MAXEYE_EVT_CHAR1_NOTIFICATION_ENABLED>", __func__);
        }
        break;
...
void maxeye_18092D_srvc_init (void)
{
    ...
    maxeye_samples_init.evt_handler=maxeye_srvc1_event_handler;
    ret=maxeye_srvc1_init(maxeye_samples_init);
    if(ret!=0)
    {
        logX("18092D sevice init=%d",ret);
    }
}
