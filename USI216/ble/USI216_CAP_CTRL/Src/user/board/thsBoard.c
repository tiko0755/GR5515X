...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
const char ABOUT[] = {"USI226_CAPCTRL_V1.0"};
...
*  static Devices
**********************************************/
#define RX_POOL_LEN	(1024)
#define TX_POOL_LEN	(1024)
...
static void onBuilded(s32 sta, void* any);
...
void thsBoardInit(void){
	...
	printS("setup userCSrv..ok\r\n");

	// initial test process, MUST place after client service setup
	infoProcInitial(&infoCSrv);
	battProcInitial(&battCSrv);
	setup_userCSRV(&userCSrv);
	procSetup_rssi();
	printS("initialize product process..ok\r\n");

	// add to cmd consumer
	consumer.append(&consumer.rsrc, brdCmd);
	consumer.append(&consumer.rsrc, test226_cmd);
	consumer.append(&consumer.rsrc, cmd_capCtrl);
	printS("add to consumer..ok\r\n");

	buildServicesProc_initial(onBuilded);	// after each build, run onBuilded
...
// call it after all services have been builded
static void onBuilded(s32 sta, void* any){
	proc_initial_userCSRV();
}
