...
#include <math.h>
#include "md5.h"
const uint32_t matrix_X[4][2] = {{0, 1}, {1, 5}, {5, 3}, {0, 7}};
const uint32_t matrix_S[4][4] = {{ 7, 12, 17, 22 },{ 5, 9 , 14, 20 },{ 4, 11, 16, 23 },{ 6, 10, 15, 21 }};
...
MD5String append_padding_bits ( char * argv )
{
    ...
    uint64_t app_length = (uint64_t)msg_length * BITS;
    MD5String string;
    ...
    string.str [ msg_length ] = (char)SINGLE_ONE_BIT;
    ...
    for (uint32_t j = 0; j < string.len; j += BLOCK_SIZE / BITS)
    {
        ...
        for ( uint8_t roundIdx = 0; roundIdx < 4; roundIdx++ )
        {
            ...
            {    
                ...
                matrix_S[ roundIdx ][ i % 4 ]);
                sIdx = ( sIdx + 3 ) % 4;
                wIdx = ( wIdx + matrix_X[ roundIdx ][ 1 ] ) & 0xF;
            }
        }
        chain[ 0 ] += state[ 0 ];
        chain[ 1 ] += state[ 1 ];
