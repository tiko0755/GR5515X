...
######################################################################################################################
#  Usage :
#       used to transfer GR551x's Keilv5 App Project to makefile file which is compiled by GNU Compiler
#  Command :
...
#   3. link file for gcc        : {APP_ROOT_DIR}}/platform/soc/linker/gcc/gcc_linker_xxx.lds
...
ifeq ($(CHIP_TYPE),0)
    LINK_SCRIPT     = ./gcc_linker_gr5515.lds
else
    LINK_SCRIPT     = ./gcc_linker_gr5513.lds
endif
        ...
        for group in target_groups.findall("Group") :
            ...
            if files :
                for file in files :
                    ...
                    if ( file_name.endswith('.c') or file_name.endswith('.cpp')  or ( ( file_name.endswith('.asm')  or file_name.endswith('.s') ) and  file_name != "startup.s") ) : 
...
class LinkerScript():
    ...
    def get_lds_path(self, path, file):
        self.file_ = g_gcc_linker + file
        file = g_lds_file + file
        count = path.count('../')
    ...
    def make(self, file_path):
        ...
        path = g_makefile_path + self.file_
...
if __name__ == "__main__":
    parm_keil_prj = None
    # is_generate_first_target = False
    is_generate_first_target = True
    which_target = 0
    is_print_help = False
    is_target_cpu_gr5515 = True
    is_gr5513_chip_type = False
    ...
    for i in range(len(sys.argv)) :
        ...
        if sys.argv[i].lower() == "-d0":
            is_generate_first_target = True
            continue
        if sys.argv[i].lower() == "-gr5513":
            is_gr5513_chip_type = True
            continue
    ...
    if (parm_keil_prj == None ) or (is_print_help == True):
        print(">>> Usuage: python keil2makefile.py  *.uvprojx  [-d0] [-gr5513] [-h/--help]")
        ...
        print(">>>>>> Note 3 :  -gr5513 - specify gr5513 chip type, otherwise gr5515 chip type will be set by default")
        print(">>>>>> Note 4 :  -h or --help - print usuage help")
        sys.exit(0)


    if (is_gr5513_chip_type == True) :
        g_target_chip_type = "1"
    else :
        g_target_chip_type = "0"
    ...
    if not ld_test.get_config_args():
        sys.exit(0)
    chip_version = ld_test.get_chip_version()
    file = ''
    if chip_version.__contains__("5513"):
        file = "gr5513.lds"
    if chip_version.__contains__("5515"):
        file =  "gr5515.lds"
    print(file)
    if ld_test.make(ld_test.get_lds_path(lds_path, file)):
