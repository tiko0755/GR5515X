...
static hexCmd_t cmdL[] = {
    ...
    {3,{0x55,0x03,0x02,0xff,0xff,0xff,0xff,0xff}},    // read FW
    {4,{0x2C,0x18,0x01,0x00,0xff,0xff,0xff,0xff}},    // SHIP MODE
};
...
static void builded_cmplt(void* argp);
...
s32 procSetup_user(bleClientSrv_dev_t* srv){
    ...
    pUserCSrv->RegEventBuilded(&pUserCSrv->rsrc, builded_cmplt);
...
APP_LOG_DEBUG("<%s> ", __func__);    
    if(pUserCSrv==NULL){    return;    }
    pUserCSrv->SetNotify(&pUserCSrv->rsrc, true, UNKNOWN1_CHAR1A_UUID, 16);
APP_LOG_DEBUG("</%s> ", __func__);
}
...
//    isProcBusy_user = 0;
    ...
    // fail        : AA-E5-04-XX
        ...
        if(p_value[3] == 0){
            ...
            app_timer_start(unknownSrvTmr_id, 1300, NULL);
        }
        else{
    ...
    // readFW response
    // success return    : 55 06 02 01 01 03
    else if((p_value[0]==0x55) && (p_value[1]==0x06) && (p_value[2]==0x02)){
        app_timer_stop(unknownSrvTmr_id);
        userNtfCB(0, &p_value[3]);  // p_value[3..5] is the ver
        userNtfCB = NULL;
        isProcBusy_user = 0;
    }
...
APP_LOG_DEBUG("<%s>", __func__);
    ...
    pUserCSrv->WriteAttr(&pUserCSrv->rsrc, UNKNOWN1_CHAR1A_UUID, UNKNOWN1_CHAR1A_UUID_LEN, cmd, len, unknownSrv_wrtCB);
...
void proc_shipmode(u16 timeout, CBx resolved){
    unknownSrv_request(cmdL[CMDL_SHIP_MODE].cmd, cmdL[CMDL_SHIP_MODE].len, timeout, resolved);
}
...
void proc_readFW(u16 timeout, CBx resolved){
    unknownSrv_request(cmdL[CMDL_READ_FW].cmd, cmdL[CMDL_READ_FW].len, timeout, resolved);
}

void proc_srv1char1_req(u8* cmd, u8 len, CBx resolve){
    usingREQ = 1;
    unknownSrv_request(cmd, len, 3000, resolve);
}
